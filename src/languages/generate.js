...
export type TTranslationCode = `;
  ...
  translations.forEach((translation) => {
    ...
    keys.forEach((key) => {
      ...
      if (
        ...
        if (!(key in baseEn)) {
          ...
          missingMessages.push(
            `${translation.code}: Value empty for key "${key}".`,
          );
        }
        return;
      }
    ...
    fs.writeFileSync(
      `./src/app/languages/${translation.code}.json`,
      JSON.stringify(json),
    );
    if (showMissing) {
  ...
  if (hasMissing) {
    console.log(
      'Show missing translations by adding "-- --missing" to the command.',
    );
  }
  console.log(
...
export function cmdAdd(code) {
  ...
  const lang = addNewLanguage(
    JSON5.parse(
      fs.readFileSync("./src/languages/_base-en.json5", { encoding: "utf-8" }),
    ),
    code,
  );
  fs.writeFileSync(
    path,
    jsBeautify.js(JSON5.stringify(lang), { indent_size: JSON_INDENT_SIZE }),
  );
...
export function cmdBuild(showMissing) {
  ...
  codes.forEach((code) => {
    translations.push({
      code,
      data: JSON5.parse(
        fs.readFileSync("./src/languages/" + code + ".json5", {
          encoding: "utf-8",
        }),
      ),
    });

...
(async () => {
  try {
    ...
    const klRecoveryManager: KlRecoveryManager | undefined =
      KL_INDEXED_DB.getIsAvailable() ? new KlRecoveryManager({}) : undefined;
    let project: TKlProject | undefined = undefined;
    try {
      const readResult: TDeserializedKlStorageProject | undefined =
        klRecoveryManager ? await klRecoveryManager.getRecovery() : undefined;
      if (readResult) {

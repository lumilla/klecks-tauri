...
export class KlCanvas {
  ...
  resize(
    w: number,
    h: number,
    algorithm: TInterpolationAlgorithm = "smooth",
  ): boolean {
    if (
      !w ||
      !h ||
  ...
  addLayer(
    ...
    const index =
      selectedIndex === undefined ? this.layers.length : selectedIndex + 1;
    ...
    const layer: TKlCanvasLayer = {
      ...
      name:
        data && data.name !== undefined
          ...
          : LANG("layers-layer") +
            " " +
            (this.layers.length + this.layerNrOffset),
  ...
  moveLayer(index: number, delta: number): void | number {
    ...
    const targetIndex = Math.max(
      0,
      Math.min(index + delta, this.layers.length),
    );
    ...
    if (!this.klHistory.isPaused()) {
      this.klHistory.push({
        ...
        layerMap: createLayerMap(
          ...
          { attributes: ["index"] },
          mergedPixelData
            ? { layerId: bottomLayer.id, attributes: "all" }
            : undefined,
        ),
      });
    }
  ...
  flip(isHorizontal: boolean, isVertical: boolean, layerIndex?: number): void {
    ...
    const targetLayer =
      layerIndex === undefined ? undefined : this.layers[layerIndex];
    this.klHistory.push({
      layerMap: createLayerMap(
        this.layers,
        targetLayer
  ...
  layerFill(
    ...
    const fill =
      "rgba(" + colorObj.r + "," + colorObj.g + "," + colorObj.b + ",1)";
    ctx.fillStyle = fill;
    ctx.fillRect(
      0,
      0,
      ...
      if (opacity === 1) {
        ...
        for (let i = 0; i < this.width * this.height; i++) {
          if (result.data[i] === 255) {
            ...
            targetData[i * 4 + 1] = BB.mix(
              targetData[i * 4 + 1],
              rgb.g,
              opacity,
            );
            targetData[i * 4 + 2] = BB.mix(
              targetData[i * 4 + 2],
              rgb.b,
              opacity,
            );
  ...
  drawShape(layerIndex: number, shapeObj: TShapeToolObject): void {
    ...
    const bounds = integerBounds(
      drawShape(targetLayer.context, shapeObj, selectionPath),
    );
  ...
  eraseLayer(p: {
    ...
    const isUniformFill =
      !p.useAlphaLock && !(p.useSelection && this.selection);
    if (!this.klHistory.isPaused()) {
      this.klHistory.push({
        layerMap: createLayerMap(this.layers, {
          layerId: targetLayer.id,
  ...
  getCompleteCanvas(
    ...
    return drawProject(
      this.getProject(),
      factor,
      maskSelection ? this.selection : undefined,
    );
  }
  ...
  setComposite(
    layerIndex: number,
    compositeObj: undefined | TLayerComposite,
  ): void {
    if (!this.layers[layerIndex]) {

...
async function loadFromImageDataStore(
  ...
  if (isBlob(result)) {
    ...
    return commonCtx.getImageData(
      0,
      0,
      commonCtx.canvas.width,
      commonCtx.canvas.height,
    );
  }
  return result;
}
...
export async function changeRecoveryId(
  oldId: number,
  newId: number,
): Promise<boolean> {
...
export async function storeRecovery(
  ...
  try {
    ...
    const sortedComposedLayers = Object.values(composed.layerMap).sort(
      sortLayerMap,
    );
    ...
    storedProjectSerialized &&
      storedImageDataIds.add(storedProjectSerialized.thumbnail);
    ...
    const imageDataList: {
      id: string;
      data: TIdb["V2"]["ImageDataStore"]["Write"];
    }[] = [];
    ...
    memoryEstimateBytes +=
      thumbImageData.data.width * thumbImageData.data.height * 4; // 4 channels, each 1 byte
    ...
    if (
      totalExceptDrawingBytes + memoryEstimateBytes >
      RECOVERY_MEMORY_LIMIT_BYTES
    ) {
      const overLimitByBytes =
        totalExceptDrawingBytes +
        memoryEstimateBytes -
        RECOVERY_MEMORY_LIMIT_BYTES;
    ...
    /*
         ...
         */
    await timeoutWrapper(
      removeOrphans(orphanCandidates),
      "storeRecovery.removeOrphans",
      8000,
    );
  }
}
...
async function getTotalMemoryExceptCurrentDrawing(
  tabId: number,
): Promise<number> {
  const ids = await getIdsFromRecoveryStore();
  let result = 0;
...
export async function getMetadata(
  ...
  return {
    ...
    thumbnail: includeThumbnail
      ? await getThumbnail(parsed.thumbnail)
      : undefined,

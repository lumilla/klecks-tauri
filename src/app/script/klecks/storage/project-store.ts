...
export function isTImageDataReference(
  input: unknown,
): input is TImageDataReference {
  return (
    typeof input === "object" &&
    input !== null &&
    "id" in input &&
...
export class ProjectStore {
  ...
  private async lowLevelRead(): Promise<TKlStorageProjectRead | undefined> {
    ...
    for (const layer of raw.layers) {
      ...
      if (isTImageDataReference(layer.blob)) {
        const readResult = (await KL_INDEXED_DB.get(
          IMAGE_DATA_STORE,
          layer.blob.id,
        )) as TIdb["V2"]["ImageDataStore"]["Read"] | undefined;
  ...
  async store(project: TKlProject): Promise<void> {
    try {
      const storageProject =
        await ProjectConverter.createStorageProject(project);

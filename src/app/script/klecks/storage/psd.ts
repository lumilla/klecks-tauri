...
function init(): void {
  ...
  psd2KlMap = Object.fromEntries(
    Object.entries(kl2PsdMap).map((a) => a.reverse()),
  );
}
...
export function readPsd(psdObj: Psd): TKlPsd {
  ...
  function prepareMask(
    ...
    const imData = groupMaskCtx.getImageData(
      0,
      0,
      maskCanvas.width,
      maskCanvas.height,
  ...
  function convertGroup(psdGroupObj: Layer): TKlPsdLayer[] {
    ...
    const groupOpacity = throwIfUndefined(
      psdGroupObj.opacity,
      "groupOpacity is undefined",
    );
        ...
        if (item.children) {
          ...
          for (let e = 0; e < convertedChildGroupItems.length; e++) {
            ...
            if (hasClipping) {
              ...
              for (
                let f = i + 1;
                f < psdGroupObj.children.length &&
                psdGroupObj.children[f].clipping;
                f++
              ) {

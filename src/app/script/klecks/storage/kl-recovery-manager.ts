...
export class KlRecoveryManager {
  ...
  private readonly crossTabChannel: CrossTabChannel = new CrossTabChannel(
    "kl-tab-communication",
  );
      ...
      if (!DEBUG_INSTANT_RECOVERY) {
        ...
        if (
          ...
          (deltaMs < SUBSEQUENT_RECOVERY_AFTER_MS ||
            changeCount - lastStoredChangeCount <
              SUBSEQUENT_RECOVERY_AFTER_CHANGES)
        ) {
          return;
        }
      }

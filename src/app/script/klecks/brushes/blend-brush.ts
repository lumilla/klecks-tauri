...
export class BlendBrush {
  ...
  private sliceBounds(bounds: TBounds): { index: number; bounds: TBounds }[] {
    ...
    touchedCells.forEach((cell, i) => {
      ...
      if (
        inCellBounds.x1 > inCellBounds.x2 ||
        inCellBounds.y1 > inCellBounds.y2
      ) {
        return;
      }
      result.push({
  ...
  private getAverage(x: number, y: number, size: number): TRgba {
    ...
    slicedBounds.forEach((slice) => {
      ...
      for (
        let i = bounds.y1, globalY = i + cellOffsetY;
        i <= bounds.y2;
        i++, globalY++
      ) {
        for (
          let e = bounds.x1,
            globalX = e + cellOffsetX,
            e2 = (i * width + bounds.x1) * 4;
          e <= bounds.x2;
          e++, globalX++, e2 += 4
        ) {
          if (
      ...
      for (
        ...
        for (
          ...
          if (this.settingLockLayerAlpha) {
            ...
            const underG =
              params.g * alphaO + data[e2 + 1] * alphaU * invAlphaO;
            const underB =
              params.b * alphaO + data[e2 + 2] * alphaU * invAlphaO;
  ...
  private continueLine(
    ...
    if (this.blending === 0) {
      ...
      if (isCoalesced) {
        ...
        const bounds = this.getDotBounds(
          avgParams[0],
          avgParams[1],
          avgParams[2],
        );
  ...
  startLine(x: number, y: number, p: number): void {
    ...
    const localOpacity = this.settingOpacityPressure
      ? this.opacity * p * p
      : this.opacity;

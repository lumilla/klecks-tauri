...
export class PenBrush {
  ...
  private updateAlphaCanvas() {
    if (
      ...
      this.settingAlphaId === ALPHA_SQUARE
    ) {
      return;
    }
  ...
  private calcOpacity(pressure: number): number {
    return (
      this.settingOpacity *
      (this.settingHasOpacityPressure ? pressure * pressure : 1)
    );
  }
  ...
  private calcScatter(pressure: number): number {
    return (
      this.settingScatter *
      this.settingSize *
      (this.settingHasScatterPressure ? pressure : 1)
    );
  }
  ...
  private continueLine(
    ...
    const drawArr: [
      ...
      number,
      number | undefined,
    ][] = []; //draw instructions. will be all drawn at once

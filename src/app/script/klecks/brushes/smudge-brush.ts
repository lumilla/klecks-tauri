...
function smudge(
  ...
  const pixel = (ai: number, bi: number, ix: number, iy: number): void => {
    ...
    const fac =
      1 -
      p.brush.opacity *
        (1 - clamp((dist - (cSize - softnessPx)) / softnessPx, 0, 1));
    ...
    if (!imageData.data[ai + 3]) {
      ...
      if (imageData.data[ai + 3] < imageData.data[bi + 3]) {
        fac2 =
          1 - (imageData.data[ai + 3] / imageData.data[bi + 3]) * (1 - fac);
      } else {
      ...
      imageData.data[bi] = Math.floor(
        BB.mix(imageData.data[ai], imageData.data[bi + 0], fac2) +
          randArr[randI],
      );
      imageData.data[bi + 1] = Math.floor(
        BB.mix(imageData.data[ai + 1], imageData.data[bi + 1], fac2) +
          randArr[randI],
      );
      imageData.data[bi + 2] = Math.floor(
        BB.mix(imageData.data[ai + 2], imageData.data[bi + 2], fac2) +
          randArr[randI],
      );
  ...
  if (p.aP.y < p.bP.y) {
    for (let y = p.size.h - 1, iy = p.bP.y + p.size.h - 1; y >= 0; y--, iy--) {
      ...
      for (
        let x = bx2 + yStart,
          x2 = bx2 + yStart - offset,
          ix = p.bP.x + p.size.w - 1;
        x >= bx1 + yStart;
        x -= 4, x2 -= 4, ix--
      ) {
        pixel(x2, x, ix, iy);
    ...
    if (p.aP.x < p.bP.x) {
      for (
        let y = p.size.h - 1, iy = p.bP.y + p.size.h - 1;
        y >= 0;
        y--, iy--
      ) {
        const yStart = (y + p.bP.y) * imageData.width * 4;
...
export class SmudgeBrush {
  ...
  private updateRedrawBounds(bounds: TBounds): void {
    ...
    this.redrawBounds = BB.updateBounds(
      this.redrawBounds,
      boundsWithinSelection,
    );
  }

  private updateCompleteRedrawBounds(
    x1: number,
    y1: number,
    x2: number,
    y2: number,
  ...
  copyFromCanvas(): void {
    ...
    touchedCells.forEach((item, i) => {
      ...
      const w =
        ((Math.min(x * CELL_SIZE + CELL_SIZE, this.copyImageData.width) - 1) %
          CELL_SIZE) +
        1;
      const h =
        ((Math.min(y * CELL_SIZE + CELL_SIZE, this.copyImageData.height) - 1) %
          CELL_SIZE) +
        1;
      ...
      for (let i = 0; i < h; i++) {
        for (
          let e = 0,
            e2 = i * w * 4,
            e3 =
              ((y * CELL_SIZE + i) * this.copyImageData.width + x * CELL_SIZE) *
              4;
          e < w;
          e++, e2 += 4, e3 += 4
        ) {
  ...
  startLine(x: number, y: number, p: number): void {
    ...
    this.copyImageData = new ImageData(
      this.context.canvas.width,
      this.context.canvas.height,
    );
    const totalCells =

...
export class SketchyBrush {
  ...
  private rand(): number {
    this.sketchySeed++;
    return (
      Math.sin(6324634.2345 * Math.cos(this.sketchySeed * 5342.3423)) * 0.5 +
      0.5
    );
  }
  ...
  startLine(x: number, y: number, pressure: number, shift?: boolean): void {
    ...
    this.selectionPath = this.selection
      ? getSelectionPath2d(this.selection)
      : undefined;
  ...
  goLine(
    ...
    p_y: number,
    pressure: number,
    preMixedColor: TRgb | undefined,
  ): void {
    if (
      !this.inputIsDrawing ||
      (p_x === this.lastInput.x && p_y === this.lastInput.y)
    ) {
      return;
    }
    ...
    if (preMixedColor) {
      ...
      if (this.settingBlending !== 0) {
        if (
          ...
          const mixed = this.mixMode[0](
            new BB.RGB(mixr, mixg, mixb),
            this.settingColor,
          );
          mixr = parseInt(
  ...
  endLine(): void {
    ...
    if (this.changedBounds) {
      const boundsWithinSelection = boundsOverlap(
        this.changedBounds,
        this.selectionBounds,
      );
      const layerData = canvasToLayerTiles(
        this.context.canvas,
  ...
  drawLineSegment(x1: number, y1: number, x2: number, y2: number): void {
    ...
    const mixed = this.mixMode[0](
      new BB.RGB(mixCol.r, mixCol.g, mixCol.b),
      this.settingColor,
    );
    mixCol.r = parseInt(
      "" +
        (this.settingBlending * mixed.r +
          this.settingColor.r * (1 - this.settingBlending)),
    );
    mixCol.g = parseInt(
      "" +
        (this.settingBlending * mixed.g +
          this.settingColor.g * (1 - this.settingBlending)),
    );
    mixCol.b = parseInt(
      "" +
        (this.settingBlending * mixed.b +
          this.settingColor.b * (1 - this.settingBlending)),
    );
    this.context.strokeStyle =
      "rgba(" +
      mixCol.r +

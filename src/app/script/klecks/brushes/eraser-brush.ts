...
import {
  getChangedTiles,
  updateChangedTiles,
} from "../history/push-helpers/changed-tiles";
...
export class EraserBrush {
  ...
  private drawDot(x: number, y: number, size: number, opacity: number): void {
    ...
    radgrad.addColorStop(
      sharpness,
      `rgba(${ERASE_COLOR}, ${ERASE_COLOR}, ${ERASE_COLOR}, ` +
        localOpacity +
        ")",
    );
    radgrad.addColorStop(
      1,
      `rgba(${ERASE_COLOR}, ${ERASE_COLOR}, ${ERASE_COLOR}, 0)`,
    );
  ...
  private continueLine(
    ...
    const bdist = Math.max(
      1,
      Math.max(0.5, 1 - this.opacity) * localSize * this.spacing,
    );
  ...
  startLine(x: number, y: number, p: number): void {
    ...
    p = Math.max(0, Math.min(1, p));
    const localOpacity = this.useOpacityPressure
      ? this.opacity * p * p
      : this.opacity;
    ...
    if (this.changedTiles.some((item) => item)) {
      this.klHistory.push(
        getPushableLayerChange(
          ...
          canvasAndChangedTilesToLayerTiles(
            this.context.canvas,
            this.changedTiles,
          ),
        ),
  ...
  drawLineSegment(x1: number, y1: number, x2: number, y2: number): void {
    ...
    const bdist = Math.max(
      1,
      Math.max(0.5, 1 - this.opacity) * this.size * this.spacing,
    );
    this.lastDot = 0;
    this.context.save();
    this.selectionPath && this.context.clip(this.selectionPath);

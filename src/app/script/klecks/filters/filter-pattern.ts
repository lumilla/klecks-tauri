...
function drawPattern(
  ...
  settings: TFilterPatternInput,
): void {
  // keep bounds in center via offset
  // because blending done towards bottom right
  const blendOffsetX = settings.blend
    ? Math.round((settings.blend * settings.width) / 2)
    : 0;
  const blendOffsetY = settings.blend
    ? Math.round((settings.blend * settings.height) / 2)
    : 0;
  ...
  if (settings.blend) {
    ...
    blendCtx.save();
    blendGradient = blendCtx.createLinearGradient(
      settings.width,
      0,
      settings.width * 2,
      0,
  ...
  context.fillStyle = throwIfNull(
    context.createPattern(finalPatternCanvas, "repeat"),
  );
  context.fillRect(
    -settings.offsetX + blendOffsetX,
    -settings.offsetY + blendOffsetY,
...
export const filterPattern = {
  getDialog(params: TFilterGetDialogParam) {
    ...
    const selectedLayerIndex = throwIfNull(
        ...
        if (previewMode === 0) {
          if (event.type === "pointerdown") {
            ...
            if (inputs.state === "select") {
              ...
              const x2 = Math.min(
                width,
                Math.max(inputs.start.x, inputs.end.x),
              );
              const y2 = Math.min(
                height,
                Math.max(inputs.start.y, inputs.end.y),
              );

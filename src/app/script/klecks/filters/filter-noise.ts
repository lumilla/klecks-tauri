...
function createNoiseSettings(input: TFilterNoiseInput): TNoiseSettings {
  const result: TNoiseSettings = BB.copyObj(
...
function createNoiseParameters(
  settings: TNoiseSettings,
): Parameters<typeof noise> {
  return [
    settings.seed,
    settings.type,
  ...
  getDialog(params: TFilterGetDialogParam) {
    ...
    const blendSelect = new Select({
      ...
      optionArr: mixModes.map((item) => {
        return item
          ? ([item, translateBlending(item)] as [TMixMode, string])
          : undefined;
      }),
    ...
    const fxPreviewRenderer = new FxPreviewRenderer({
      ...
      onUpdate: (fxCanvas, transform) => {
        ...
        settings.offsetX =
          (context.canvas.width / 2) * transform.scaleX + transform.x;
        settings.offsetY =
          (context.canvas.height / 2) * transform.scaleY + transform.y;
        return fxCanvas.noise(...createNoiseParameters(settings));
      },
      postMix: {
        opacity: noiseInput.opacity,
        operation:
          noiseInput.channels === "alpha"
            ? "destination-out"
            : noiseInput.mixModeStr,
      },

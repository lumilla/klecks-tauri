...
export function testComposedLayerHasTransparency(
  layer: THistoryEntryLayerComposed,
): boolean {
...
function drawTransform(
  ...
  if (selection) {
    ...
    if (!doClone) {
      ...
      ctx.globalCompositeOperation = isTransparentBg
        ? "destination-out"
        : "source-atop";
      ctx.fillStyle = "white";
  ...
  getDialog(params: TFilterGetDialogParam) {
    ...
    if (!boundsObj) {
      return {
        error: LANG(
          selection
            ? "filter-transform-empty-selection"
    ...
    checkboxWrapper.append(
      constrainCheckbox.getElement(),
      snappingCheckbox.getElement(),
    );
    ...
    const previewLayerArr: TProjectViewportProject["layers"] = [];
    {
      for (let i = 0; i < layers.length; i++) {
        previewLayerArr.push({
          image:
            i === selectedLayerIndex ? previewCanvas : layers[i].context.canvas,
    ...
    const preview = new Preview({
      ...
      hasEditMode: true,
      onModeChange: (m) => {
        freeTransform.getElement().style.pointerEvents =
          m === "edit" ? "" : "none";

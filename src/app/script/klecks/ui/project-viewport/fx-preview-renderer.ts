...
export type TFxPreviewRendererParams = {
  original:
    | Exclude<CanvasImageSource, VideoFrame | HTMLOrSVGImageElement>
    | HTMLImageElement;
...
export class FxPreviewRenderer {
  ...
  render: TProjectViewportLayerFunc = (
    viewportTransform,
    viewportWidth,
    viewportHeight,
  ) => {
    ...
    let resultTransform = compose(
      ...
      translate(
        clippedViewportRect.x - padding,
        clippedViewportRect.y - padding,
      ),
    );
    ...
    if (viewportTransform.scaleX > 1) {
      ...
      canvasBottomRight.x = Math.min(
        this.original.width,
        Math.ceil(canvasBottomRight.x),
      );
      canvasBottomRight.y = Math.min(
        this.original.height,
        Math.ceil(canvasBottomRight.y),
      );
    ...
    if (
      ...
      if (this.selection && (!this.maskTexture || propsDidChange)) {
        ...
        const transformedSelection = transformMultiPolygon(
          this.selection,
          transformMatrix,
        );

...
import {
  TMetaTransform,
  toMetaTransform,
} from "../../../../bb/transform/to-meta-transform";
...
export function blendTransform(
  ...
  if (
    ...
    (currentTransform.scale === targetTransform.scale ||
      Math.abs(currentTransform.scale - targetTransform.scale) <
        0.08 * targetTransform.scale)
  ) {
    return {
      ...targetTransform,
    };
  ...
  if (BB.Vec2.dist(currentMeta.canvasP, targetMeta.canvasP) < 1) {
    ...
    const mixedMeta: TMetaTransform = {
      ...
      scale: BB.mix(currentTransform.scale, targetTransform.scale, easeFactor),
      angleDeg: BB.mix(
        closerCurrentAngleDeg,
        targetTransform.angleDeg,
        easeFactor,
      ),
    };
  ...
  const mixedScale = BB.mix(
    currentTransform.scale,
    targetTransform.scale,
    easeFactor,
  );
  const mixedAngleDeg = BB.mix(
    currentTransform.angleDeg,
    targetTransform.angleDeg,
    easeFactor,
  );
  ...
  const rotatedOffsetX =
    offsetX * Math.cos(mixedAngleRad) - offsetY * Math.sin(mixedAngleRad);
  const rotatedOffsetY =
    offsetX * Math.sin(mixedAngleRad) + offsetY * Math.cos(mixedAngleRad);

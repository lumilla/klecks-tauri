...
export function newImageDialog(p: {
  ...
  function updateRatio(): void {
    ...
    heightInput.value =
      "" + Math.min(maxCanvasSize, parseInt(heightInput.value));
    ...
    for (let i = 0; i < commonRatios.length; i++) {
      if (
        Math.abs(
          commonRatios[i][0] / commonRatios[i][1] -
            reducedArr[0] / reducedArr[1],
        ) < closestDistance
      ) {
    ...
    if (closestDistance > 0 && closestDistance < 0.005) {
      ...
      ratioWrapper.innerText =
        LANG("new-ratio") + ": " + reducedArr[0] + ":" + reducedArr[1];
    }
    ...
    previewWrapper.style.backgroundSize =
      Math.round(Math.max(4, 60 * (w / realw))) + "px";
  }

...
import {
  renderText,
  TRenderTextParam,
} from "../../../image-operations/render-text";
...
export class TextToolViewportUI {
  ...
  private readonly onDarkChange = () => {
    this.checkerPattern = throwIfNull(
      this.previewCtx.createPattern(
        BB.createCheckerCanvas(8, THEME.isDark()),
        "repeat",
      ),
    );
    this.render();
  ...
  private move(x: number, y: number): void {
    const rotated = BB.rotate(
      x,
      y,
      (-this.rotationSlider.getValue() / Math.PI) * 180,
    );
  ...
  render(): void {
    ...
    bounds.width += 6;
    bounds.height += 6;
    const rotatedXY = BB.rotate(
      bounds.x,
      bounds.y,
      (-angleRad / Math.PI) * 180,
    );
    const rotatedWH = BB.rotate(
      bounds.width,
      bounds.height,
      (-angleRad / Math.PI) * 180,
    );
    const centerX =
      this.text.x + rotatedXY.x + rotatedWH.x / 2 + transformedOffset.x;
    const centerY =
      this.text.y + rotatedXY.y + rotatedWH.y / 2 + transformedOffset.y;
    ...
    this.targetCtx.drawImage(
      this.layerArr[this.layerIndex].canvas,
      -centerX,
      -centerY,
    );
...

    {
      ...
      if (this.scale >= 4) {
        ...
        this.layersCtx.imageSmoothingQuality =
          this.scale >= 1 ? "low" : "medium";
      }
      ...
      this.layersCtx.globalCompositeOperation =

...
export class KlSlider {
  ...
  private valueToSliderValue(value: number): number {
    if (this.useSpline && this.splineInterpolator) {
      return (
        this.splineInterpolator.findX(value, Math.floor(this.resolution)) || 0
      );
    }
  ...
  private updateLabel(): void {
    ...
    displayValue = this.formatFunc
      ? this.formatFunc(displayValue)
  ...
  private updateEnable(): void {
    this.sliderWrapperEl.classList.toggle(
  ...
  constructor(p: {
    ...
    if (!p.toValue !== !p.toDisplayValue) {
      throw new Error(
        "both or neither have to be set, toValue and toDisplayValue",
      );
    }
    this.displayValueToValue = p.toValue
      ? p.toValue
      : (displayValue) => displayValue;
    this.valueToDisplayValue = p.toDisplayValue
      ? p.toDisplayValue
      : (value) => value;
    ...
    const onPointer = (event: TPointerEvent) => {
      ...
      if (event.type === "pointerdown") {
        ...
        this.sliderWrapperEl.className =

...
import classes from "./browser-storage-ui.module.scss";
...
export class BrowserStorageUi {
  ...
  private readonly storeListener: TProjectStoreListener =
    {} as TProjectStoreListener;
  ...
  private options:
    | { hideClearButton?: boolean; isFocusable?: boolean }
    | undefined;
    ...
    showModal({
      ...
      callback: async (result) => {
        ...
        try {
          ...
          setTimeout(() => {
            throw new Error(
              "storage-ui: failed to clear browser storage, " + e,
            );
          }, 0);
        }
      },

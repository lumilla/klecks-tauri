...
export class CropCopy {
  ...
  constructor(p: TCropCopyParams) {
    ...
    this.preview = new Preview({
      ...
      onModeChange: (mode) => {
        this.mode = mode;
        this.preview.getElement().style.pointerEvents =
          mode === "edit" ? "none" : "";
    ...
    const genCrop = (p1: TVector2D, p2: TVector2D): TRect => {
      ...
      origBottomRightP.x = clamp(
        Math.ceil(origBottomRightP.x),
        0,
        this.canvas.width,
      );
      origBottomRightP.y = clamp(
        Math.ceil(origBottomRightP.y),
        0,
        this.canvas.height,
      );
      return {
    ...
    pointerChain.setChainOut((event) => {
      if (event.type === "pointerdown" && event.button === "left") {
        ...
        if (startCrop) {
          ...
          this.cropRect.x =
            startCrop.x +
            Math.round((event.relX - startP!.x) / transform.scale);
          this.cropRect.y =
            startCrop.y +
            Math.round((event.relY - startP!.y) / transform.scale);

...
export class LayerPreview {
  ...
  private drawLargeCanvas(): void {
    ...
    css(this.largeCanvasWrapper, {
      top:
        Math.max(
          10,
          bounds.top + this.height / 2 - this.largeCanvas.height / 2,
        ) + "px",
    });
  }
  ...
  constructor(p: {
    ...
    this.largeCanvasWrapper = BB.el({
      ...
      css: {
        ...
        transition:
          "opacity " + largeCanvasAnimationDurationMs + "ms ease-in-out",
        userSelect: "none",
        display: "block",
  ...
  setIsVisible(b: boolean): void {
    ...
    this.contentWrapperEl.style.display = this.isPreviewVisible
      ? "flex"
      : "none";

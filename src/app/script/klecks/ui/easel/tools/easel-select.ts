...
function transformFreeTransformViaMatrix(
  ...
  const tlBefore = applyToPoint(toCanvasMatrix, {
    x: -width / 2,
    y: -height / 2,
  });
  const trBefore = applyToPoint(toCanvasMatrix, {
    x: width / 2,
    y: -height / 2,
  });
  const blBefore = applyToPoint(toCanvasMatrix, {
    x: -width / 2,
    y: height / 2,
  });
...
export class EaselSelect implements TEaselTool {
    ...
    this.freeTransform = new FreeTransform({
      ...
      callback: (transform) => {
        ...
        const matrix = compose(
          freeTransformMatrix,
          inverse(this.initialTransformMatrix),
        );
  ...
  updateInitialTransformMatrix(): void {
    ...
    this.initialTransformMatrix = freeTransformToMatrix(
      transform,
      this.selectionBounds!,
    );
  }
  ...
  setTransform(matrix: Matrix): void {
    const newFreeTransform = transformFreeTransformViaMatrix(
      this.initialFreeTransform,
      matrix,
    );

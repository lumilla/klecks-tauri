...
export type TEaselParams<GToolId extends string> = {
  ...
  ) => void; // whenever Viewport changes
...
export class Easel<GToolId extends string> {
  ...
  private readonly easelInterface: TEaselInterface = {
    ...
    setTransform: (transform, isImmediate) =>
    ...
    setRenderedSelection: (selection) =>
      this.selectionRenderer.setRenderedSelection(selection),
  ...
  constructor(p: TEaselParams<GToolId>) {
      ...
      onPinch: (event) => {
        if (event.type === "move") {
          ...
          if (!this.pinchInitialTransform) {
            ...
            angleIsExtraSticky =
              this.pinchInitialTransform.angleDeg % 180 === 0;
          }

          let newAngleDeg =
            this.pinchInitialTransform.angleDeg +
            (event.angleRad / Math.PI) * 180;
    ...
    this.keyListener = new KeyListener({
      onDown: (keyStr, e, comboStr, isRepeat) => {
        ...
        if (
          this.keyListener.comboOnlyContains([
            "shift",
            "left",
            "right",
            "up",
    ...
    this.svgEl.append(
      ...
      ...Object.values<TEaselTool>(this.toolsMap).map((item) =>
        item.getSvgElement(),
      ),
    );
  ...
  scale(factor: number, viewportX?: number, viewportY?: number): void {
    ...
    const metaTransform = toMetaTransform(before, {
      x: viewportX,
      y: viewportY,
    });
    metaTransform.scale = BB.clamp(
  ...
  fitTransform(isImmediate?: boolean): boolean {
    ...
    const isPositionChanged =
      transform.x !== oldTransform.x || transform.y !== oldTransform.y;
    const isScaleOrAngleChanged =
      transform.scale !== oldTransform.scale ||
      transform.angleDeg !== oldTransform.angleDeg;

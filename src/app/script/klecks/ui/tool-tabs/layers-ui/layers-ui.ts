...
export class LayersUi {
  ...
  private readonly moreDropdown: DropdownMenu<
    "clear-layer" | "advanced-merge" | "merge-all"
  >;
  ...
  private move(oldSpotIndex: number, newSpotIndex: number): void {
    ...
    this.klCanvas.moveLayer(
      this.selectedSpotIndex,
      newSpotIndex - oldSpotIndex,
    );
  ...
  private posToSpot(p: number): number {
    let result = parseInt(
      "" + (p / (this.layerHeight + this.layerSpacing) + 0.5),
    );
  ...
  private renameLayer(layerSpot: number): void {
    renameLayerDialog(
      this.parentEl,
      this.klCanvas.getLayerOld(layerSpot)!.name,
      (newName) => {
        if (
          newName === undefined ||
          newName === this.klCanvas.getLayerOld(layerSpot)!.name
        ) {
          return;
        }
    ...
    const createLayerEntry = (index: number): void => {
      ...
      const dragEventHandler = (event: TPointerEvent) => {
        if (event.type === "pointerdown" && event.button === "left") {
          ...
          this.updateLayersVerticalPosition(
            layer.spot,
            this.posToSpot(layer.posY),
          );
        }
    ...
    const createButtons = () => {
      ...
      const async = () => {
        ...
        this.duplicateBtn.innerHTML =
          "<img src='" + duplicateLayerImg + "' height='20'/>";
        this.mergeBtn.innerHTML =
          "<img src='" + mergeLayerImg + "' height='20'/>";
        this.removeBtn.innerHTML =
          "<img src='" + removeLayerImg + "' height='20'/>";
  ...
  activateLayer(spotIndex: number): void {
    if (spotIndex < 0 || spotIndex > this.layerElArr.length - 1) {
      throw (
        ...
        spotIndex +
        ", layerElArr.length " +
        this.layerElArr.length
      );
    }
    this.selectedSpotIndex = spotIndex;
    this.modeSelect.setValue(
      this.klCanvasLayerArr[this.selectedSpotIndex].mixModeStr,
    );

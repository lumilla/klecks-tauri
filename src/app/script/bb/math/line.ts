...
export class PointLine {
  ...
  getAtDist(dist: number): TVector2D {
    ...
    for (
      ;
      remainder > this.segmentArr[i].length && i < this.segmentArr.length - 2;
      i++
    ) {
...
export class BezierLine {
  ...
  add(
    ...
    if (this.pointArr.length === 1) {
      ...
      this.pointArr[0].dir = Vec2.nor(
        Vec2.sub(this.pointArr[1], this.pointArr[0]),
      );

...
export function drawTransformedImageOnCanvas(
  ...
  ctx.drawImage(
    ...
    transformImage.height,
  );

  ctx.restore();
}

export const createCheckerCanvas = function (
  size: number,
  isDark?: boolean,
): HTMLCanvasElement {
...
export function resizeCanvas(
  ...
  if (w <= canvas.width && h <= canvas.height) {
    ...
    tmp2.width =
      base2.oldWidthEx > base2.newWidthEx ? Math.pow(2, base2.oldWidthEx) : w;
    tmp2.height =
      base2.oldHeightEx > base2.newHeightEx
        ? Math.pow(2, base2.oldHeightEx)
        : h;
    ...
    for (; ew > base2.newWidthEx || eh > base2.newHeightEx; ew--, eh--) {
      ...
      const newHeight =
        eh > base2.newHeightEx ? currentHeight / 2 : currentHeight;
...
export function htmlCanvasToBlobAsync(
  canvas: HTMLCanvasElement,
  mimeType: string,
): Promise<Blob> {

...
export class PinchZoomer {
  ...
  private processEvent(event: TPointerEvent): void {
    ...
    if (event.type === "pointerdown") {
      if (this.gestureObj) {
        ...
        if (
          !this.setupFailTimeout(event.time + this.untilSecondFingerDurationMs)
        ) {
          // time ran out -> fail
          this.fail();
          return;
    ...
    if (event.type === "pointermove" && event.pointerType === "touch") {
      ...
      if (!this.gestureObj.isInProgress) {
        ...
        if (
          !(
            "downRelX" in touchPointerObj &&
            touchPointerObj.downRelX !== undefined
          ) ||
          !(
            "downRelY" in touchPointerObj &&
            touchPointerObj.downRelY !== undefined
          )
        ) {
          this.fail();
          return;
    ...
    if (event.type === "pointerup") {
      ...
      if (event.pointerType === "touch") {
        ...
        for (; i < this.gestureObj.touchPointerArr.length; i++) {
          if (
            this.gestureObj.touchPointerArr[i].pointerId === event.pointerId
          ) {
  ...
  private beginPinch(gestureObj: TPinchGesture): void {
    ...
    if (this.pincherArr.length === 1) {
      ...
      event.relX =
        0.5 * (this.pincherArr[0].downRelX + this.pincherArr[1].downRelX);
      event.relY =
        0.5 * (this.pincherArr[0].downRelY + this.pincherArr[1].downRelY);
    }

...
const globalKey = ((): TGlobalKey => {
  ...
  // { space: false, ... }
  const isDownObj: TIsDown = Object.entries(keyStrToKeyObj).reduce(
    (acc, [key]) => {
      acc[key] = false;
      return acc;
    },
  ...
  // event.key to keyStr
  // { ArrowLeft: 'left', ... }
  const keyToKeyStrObj = Object.entries(keyStrToKeyObj).reduce(
    (acc, [key, code]) => {
...
export function sameKeys(comboAStr: string, comboBStr: string): boolean {
  return (
    comboAStr.split("+").sort().join("+") ===
    comboBStr.split("+").sort().join("+")
  );
}

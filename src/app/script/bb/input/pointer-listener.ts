...
const pointerDownEvt = (
  HAS_POINTER_EVENTS ? "pointerdown" : "mousedown"
) as "pointerdown";
const pointerMoveEvt = (
  HAS_POINTER_EVENTS ? "pointermove" : "mousemove"
) as "pointermove";
const pointerUpEvt = (
  HAS_POINTER_EVENTS ? "pointerup" : "mouseup"
) as "pointerup";
const pointerCancelEvt = (
  HAS_POINTER_EVENTS ? "pointercancel" : "mousecancel"
) as "pointercancel";
const pointerLeaveEvt = (
  HAS_POINTER_EVENTS ? "pointerleave" : "mouseleave"
) as "pointerleave";
const pointerEnterEvt = (
  HAS_POINTER_EVENTS ? "pointerenter" : "mouseenter"
) as "pointerenter";
...
function correctPointerEvent(
  ...
  const correctedObj: TCorrectedPointerEvent = {
    ...
    pressure: pressureNormalizer.normalize(
      event.pressure,
      event.type,
      event.pointerType,
    ),
  ...
  if ("pointerId" in event) {
    if ("pressure" in event && event.buttons !== 0) {
      ...
      if (
        ...
        (event.pressure === 0.5 || event.pressure === 0)
      ) {
        correctedObj.pressure = 1;
        customPressure = 1;
      }
  ...
  const pointerObj: TPointer =
    getPointer(correctedObj) || addPointer(correctedObj);
  ...
  for (let i = 0; i < coalescedEventArr.length; i++) {
    ...
    correctedObj.coalescedArr.push({
      ...
      movementX:
        pointerObj.lastPageX === null
          ? 0
          : eventItem.pageX - pointerObj.lastPageX,
      movementY:
        pointerObj.lastPageY === null
          ? 0
          : eventItem.pageY - pointerObj.lastPageY,
      timeStamp:
  ...
  correctedObj.movementX =
    totalLastX === null ? 0 : pointerObj.lastPageX - totalLastX;
  correctedObj.movementY =
    totalLastY === null ? 0 : pointerObj.lastPageY - totalLastY;
...
export class PointerListener {
  ...
  private readonly onPointerCallback:
    | undefined
    | ((pointerEvent: TPointerEvent) => void);
  private readonly onWheelCallback:
    | undefined
    | ((wheelEvent: TWheelEvent) => void);
  private readonly onEnterLeaveCallback:
    | undefined
    | ((isOver: boolean) => void);
  ...
  private readonly onTouchMoveScribbleFix:
    | ((e: TouchEvent) => void)
    | undefined;
  private readonly windowOnPointerMove:
    | ((event: PointerEvent) => void)
    | undefined;
  private readonly windowOnPointerUp:
    | ((event: PointerEvent) => void)
    | undefined;
  private readonly windowOnPointerLeave:
    | ((event: PointerEvent) => void)
    | undefined;
  ...
  private createPointerOutEvent(
    ...
    const result: TPointerEvent = {
      ...
      relX:
        correctedEvent.clientX - bounds.left + this.targetElement.scrollLeft,
      ...
      if (correctedEvent.coalescedArr.length > 1) {
        ...
        for (let i = 0; i < correctedEvent.coalescedArr.length; i++) {
          ...
          result.coalescedArr.push({
            ...
            relX:
              coalescedItem.clientX -
              bounds.left +
              this.targetElement.scrollLeft,
            relY:
              coalescedItem.clientY - bounds.top + this.targetElement.scrollTop,
  ...
  constructor(p: TPointerListenerParams) {
    ...
    this.wheelCleaner = p.useDirtyWheel
      ? undefined
      : new WheelCleaner(finalizeWheelEvent);
    ...
    if (this.onPointerCallback) {
      this.onPointerMove = (event: PointerEvent) => {
        ...
        const outEvent = this.createPointerOutEvent(
          "pointermove",
          correctedEvent,
        );

...
export class KlAppSelect {
    ...
    this.selectUi = new SelectUi({
      onChangeMode: (mode) => {
        ...
        if (this.selectMode === "select") {
          ...
          this.initialTransform.backgroundIsTransparent =
      ...
      transform: {
        ...
        onScale: (factor) => {
          ...
          if (
            factor < 1 &&
            (factor * before.width < 1 || factor * before.height < 1)
          ) {
            return;
          }
  ...
  discardTransform(): boolean {
    if (this.selectMode === "transform") {
      ...
      this.targetLayerIndex = throwIfNull(
        this.klCanvas.getLayerIndex(currentCanvas),
      );
      this.selectUi.setMode("select"); // this triggers selectUi.onMode
      return true;
    }

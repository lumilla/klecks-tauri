...
import {
  EASEL_MAX_SCALE,
  EASEL_MIN_SCALE,
} from "../klecks/ui/easel/easel.config";
...
export class KlApp {
  ...
  private updateCollapse(isInitial?: boolean): void {
    ...
    if (this.uiWidth < this.collapseThreshold) {
      ...
      if (this.mobileUi.getToolspaceIsOpen()) {
        ...
        this.easel.setSize(
          Math.max(0, this.uiWidth - this.toolWidth),
          this.uiHeight,
        );
  ...
  private updateBottomBar(): void {
    ...
    const isVisible =
      this.toolspaceInner.scrollHeight + 40 < window.innerHeight;
  ...
  private updateUi(): void {
    this.toolspace.classList.toggle(
      "kl-toolspace--left",
      this.uiLayout === "left",
    );
    this.toolspace.classList.toggle(
  ...
  constructor(p: TKlAppParams) {
    ...
    const projectStore = KL_INDEXED_DB.getIsAvailable()
      ? new ProjectStore()
      : undefined;
    this.rootEl = BB.el({
      className: "g-root",
    ...
    const getNextBrushId = (): string => {
      ...
      const keyArr = Object.keys(brushUiMap).filter(
    ...
    drawEventChain.setChainOut(((event: TDrawEvent) => {
      ...
      if (event.type === "line") {
        currentBrushUi
          .getBrush()
          .drawLineSegment(event.x0, event.y0, event.x1, event.y1);
    ...
    this.easel = new Easel({
      ...
      onTransformChange: (transform, isScaleOrAngleChanged) => {
        ...
        this.toolspaceToolRow.setEnableZoomIn(
          transform.scale !== EASEL_MAX_SCALE,
        );
        this.toolspaceToolRow.setEnableZoomOut(
          transform.scale !== EASEL_MIN_SCALE,
        );
    ...
    const keyListener = new BB.KeyListener({
      onDown: (keyStr, event, comboStr) => {
        ...
        const isDrawing =
          this.lineSanitizer.getIsDrawing() || this.easel.getIsLocked();
        if (isDrawing) {
          return;
        }
        ...
        if (comboStr === "b") {
          ...
          updateMainTabVisibility();
          brushTabRow.open(
            prevMode === "brush" ? getNextBrushId() : currentBrushId,
          );
        }
        if (comboStr === "g") {
          event.preventDefault();
          ...
          KL.popup({
            ...
            callback: async (result) => {
              ...
              const overlay = BB.el({
                ...
                content:
                  '<div class="spinner"></div> ' + LANG("submit-submitting"),
              });
    ...
    this.klColorSlider.setHeight(
      Math.max(163, Math.min(400, this.uiHeight - 505)),
    );
    ...
    const shapeTool = new KL.ShapeTool({
      onShape: (isDone, x1, y1, x2, y2, angleRad) => {
        ...
        if (shapeUi.getShape() === "line") {
          ...
          shapeObj.isAngleSnap =
            shapeUi.getIsSnap() || keyListener.isPressed("shift");
        } else {
          shapeObj.isFixedRatio =
            shapeUi.getIsFixed() || keyListener.isPressed("shift");
    ...
    const onOpenBrowserStorage = async () => {
      ...
      if (
        meta &&
        this.klHistory.getComposed().projectId.value !== meta.projectId
      ) {
        let doOpen = true;
